...
import { xaiFactory } from '../../../src/adapters/openai/index.js';
...
export interface XaiMockSpies {
  create: Mock;
  modelsList: Mock;
}

export function createXaiHarness(mocks: XaiMockSpies): ContractHarness {
  ...
  return {
    ...
    authFor: (kind: AuthKind): AuthSpec => {
      if (kind === 'apiKey') return { kind: 'apiKey', apiKey: 'xai-test' };
      throw new Error(`Xai harness does not provide auth for kind '${kind}'`);
    },
    unsupportedAuth: {
      kind: 'bedrockIam',
      accessKeyId: 'x',
