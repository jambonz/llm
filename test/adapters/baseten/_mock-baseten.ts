...
import { basetenFactory } from '../../../src/adapters/openai/index.js';
...
export interface BasetenMockSpies {
  create: Mock;
  modelsList: Mock;
}

export function createBasetenHarness(mocks: BasetenMockSpies): ContractHarness {
  ...
  function programMock(scenario: ContractScenario): void {
    ...
    mocks.modelsList.mockResolvedValue({
      data: [
        { id: 'zai-org/GLM-4.7', object: 'model' },
  ...
  return {
    vendor: 'baseten',
    factory: basetenFactory,
    authFor: (kind: AuthKind): AuthSpec => {
      if (kind === 'apiKey') return { kind: 'apiKey', apiKey: 'bt-test' };
      throw new Error(`Baseten harness does not provide auth for kind '${kind}'`);
    },
    unsupportedAuth: {
      kind: 'bedrockIam',
      accessKeyId: 'x',
    ...
    toolCapableModel: 'zai-org/GLM-4.7',
